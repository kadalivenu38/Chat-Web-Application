...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 px-4 py-3 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 
      ...
      {/* Search chats */}
      <div className='flex items-center gap-2 p-2.5 mt-2 border border-gray-400 dark:border-white/20 rounded-md'>
          ...
          chats.filter((chat) => chat.messages[0] ?
            ...
            : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
              <div onClick={() => { navigate('/'); setSelectedChat(chat); setIsMenuOpen(false) }}
                key={chat._id} className='p-1 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15
      ...
      {/* Community Images */}
      <div onClick={() => { navigate('/community'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-2 mt-5 
        border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-2 mt-2 border 
      ...
      {/* User Account */}
      <div className='flex items-center gap-3 p-2 mt-4 border border-gray-300 dark:border-white/15
